...
export default function MerchantControlHub() {
  ...
  const fetchCatalogStatus = async () => {
    ...
    try {
      const r = await axios.get(
        `${API_URL}/admin/catalog-status/${merchantId}`,
        { headers: ah() },
      );
  ...
  const handleFullSync = async () => {
    if (
      !confirm(
        "Full sync fetches ALL Shopify data. This runs in background. Continue?",
      )
    )
      return;
    setLoading("fullsync");
  ...
  const handleRegisterWebhooks = async () => {
    ...
    try {
      ...
      setWabaSubResult(r.data.wabaSubscription || null);
      const wabaSub = r.data.wabaSubscription;
      const wabaMsg = wabaSub
        ? `\n\nMeta WABA Subscription: ${wabaSub.status.toUpperCase()} — ${wabaSub.reason}`
        : "";
      alert(r.data.message + wabaMsg);
    } catch (e: any) {
      alert(e.response?.data?.message || "Failed");
    } finally {
      setLoading(null);
  ...
  const handleSendMPM = async () => {
    ...
    if (mpmMode === "catalog" && !mpmToPhone) {
      alert("Phone number required for catalog message");
      return;
    }
    setMpmSending(true);
    try {
      if (mpmMode === "catalog") {
        ...
        const sections = mpmSections
          .filter((s: any) => s.products.trim())
          .map((s: any) => ({
            title: s.title || "Products",
            product_items: s.products
              .split(",")
              .map((id: string) => ({ product_retailer_id: id.trim() }))
              .filter((p: any) => p.product_retailer_id),
          }));
        const r = await axios.post(
          `${API_URL}/admin/send-mpm`,
          {
            ...
            bodyVariables: mpmBodyVars
              ? mpmBodyVars.split(",").map((v: string) => v.trim())
              : [],
      ...
      <header className="h-14 md:h-16 bg-slate-900/80 backdrop-blur border-b border-white/5 flex items-center px-3 md:px-8 gap-2 md:gap-4 sticky top-0 z-10">
        ...
        <div className="flex items-center gap-1.5 md:gap-2 shrink-0">
          ...
          {redFlags?.qualityRating && redFlags.qualityRating !== "UNKNOWN" && (
            <div
              className={`hidden sm:flex items-center gap-1 px-2 py-1 rounded-xl border text-[10px] font-extrabold ${redFlags.qualityRating === "GREEN" ? "bg-green-500/10 border-green-500/20 text-green-400" : redFlags.qualityRating === "YELLOW" ? "bg-yellow-500/10 border-yellow-500/20 text-yellow-400 animate-pulse" : "bg-red-500/10 border-red-500/20 text-red-400 animate-pulse"}`}
            >
              <span
                className={`w-1.5 h-1.5 rounded-full ${redFlags.qualityRating === "GREEN" ? "bg-green-400" : redFlags.qualityRating === "YELLOW" ? "bg-yellow-400" : "bg-red-400"}`}
              />
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-6">
        {activeTab === "overview" && (
          <OverviewTab
            ...
            campaigns={campaigns}
            redFlags={redFlags}
            redFlagsLoading={redFlagsLoading}
            fetchRedFlags={fetchRedFlags}
            wabaInfo={wabaInfo}
            wabaLoading={wabaLoading}
            fetchWabaInfo={fetchWabaInfo}
            roiReport={roiReport}
            roiLoading={roiLoading}
            roiDays={roiDays}
            roiFee={roiFee}
            roiCopied={roiCopied}
            setRoiDays={setRoiDays}
            setRoiFee={setRoiFee}
            generateRoiReport={generateRoiReport}
            copyRoiMessage={copyRoiMessage}
            aiAutoReply={aiAutoReply}
            setAiAutoReply={setAiAutoReply}
            aiKnowledgeBase={aiKnowledgeBase}
            setAiKnowledgeBase={setAiKnowledgeBase}
            aiFallbackMessage={aiFallbackMessage}
            setAiFallbackMessage={setAiFallbackMessage}
            aiSaving={aiSaving}
            ...
            setShopifySecret={setShopifySecret}
            storeUrl={storeUrl}
        ...
        {activeTab === "inbox" && (
          <InboxTab
            ...
            inboxMessagesLoading={inboxMessagesLoading}
            timeLeft={timeLeft}
            replyText={replyText}
        ...
        {activeTab === "flows" && (
          <FlowsTab
            ...
            flowDrafts={flowDrafts}
            setFlowDrafts={setFlowDrafts}
            fetchAll={fetchAll}
        ...
        {activeTab === "campaign" && (
          <CampaignTab
            ...
            campMetaTemplate={campMetaTemplate}
            setCampMetaTemplate={setCampMetaTemplate}
            campMetaLang={campMetaLang}
            setCampMetaLang={setCampMetaLang}
            campDiscountCode={campDiscountCode}
            setCampDiscountCode={setCampDiscountCode}
            campScheduleMode={campScheduleMode}
            setCampScheduleMode={setCampScheduleMode}
            campScheduledAt={campScheduledAt}
            setCampScheduledAt={setCampScheduledAt}
            campCustomerFilter={campCustomerFilter}
            setCampCustomerFilter={setCampCustomerFilter}
        ...
        {activeTab === "mpm" && (
          <ProductsTab
            ...
            customerTotal={customerTotal}
            mpmMode={mpmMode}
            setMpmMode={setMpmMode}
            mpmTemplate={mpmTemplate}
            setMpmTemplate={setMpmTemplate}
            mpmLang={mpmLang}
            setMpmLang={setMpmLang}
            mpmBodyVars={mpmBodyVars}
            setMpmBodyVars={setMpmBodyVars}
            mpmThumbnailId={mpmThumbnailId}
            setMpmThumbnailId={setMpmThumbnailId}
            mpmSections={mpmSections}
            setMpmSections={setMpmSections}
            mpmCustomerFilter={mpmCustomerFilter}
            setMpmCustomerFilter={setMpmCustomerFilter}
            mpmToPhone={mpmToPhone}
            setMpmToPhone={setMpmToPhone}
            mpmSending={mpmSending}
            mpmBodyText={mpmBodyText}
            setMpmBodyText={setMpmBodyText}
            mpmFooterText={mpmFooterText}
            setMpmFooterText={setMpmFooterText}
            catalogStatus={catalogStatus}
            catalogChecking={catalogChecking}
            fetchCatalogStatus={fetchCatalogStatus}
            handleSendMPM={handleSendMPM}
          />
        )}
        ...
        {activeTab === "customers" && (
          <CustomersTab
            ...
            addCustPhone={addCustPhone}
            setAddCustPhone={setAddCustPhone}
        ...
        {activeTab === "credentials" && (
          <CredentialsTab
            loading={loading}
            credStoreUrl={credStoreUrl}
            setCredStoreUrl={setCredStoreUrl}
            credShopifyToken={credShopifyToken}
            setCredShopifyToken={setCredShopifyToken}
            credShopifySecret={credShopifySecret}
            setCredShopifySecret={setCredShopifySecret}
            credClientId={credClientId}
            setCredClientId={setCredClientId}
            credClientSecret={credClientSecret}
            setCredClientSecret={setCredClientSecret}
            credMetaPhoneId={credMetaPhoneId}
            setCredMetaPhoneId={setCredMetaPhoneId}
            credMetaWabaId={credMetaWabaId}
        ...
        {activeTab === "templates" && (
          <TemplatesTab
            ...
            tmplBody={tmplBody}
            setTmplBody={setTmplBody}
            tmplHeader={tmplHeader}
            setTmplHeader={setTmplHeader}
            tmplFooter={tmplFooter}
            setTmplFooter={setTmplFooter}
            tmplCategory={tmplCategory}
            setTmplCategory={setTmplCategory}
            tmplLanguage={tmplLanguage}
            setTmplLanguage={setTmplLanguage}
            tmplButtons={tmplButtons}
            setTmplButtons={setTmplButtons}
            tmplFilter={tmplFilter}
            setTmplFilter={setTmplFilter}
