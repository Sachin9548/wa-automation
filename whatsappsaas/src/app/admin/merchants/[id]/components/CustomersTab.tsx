...
interface CustomersTabProps {
  ...
  customerStats: { noPhoneCount: number; waInvalidCount: number; abandonedCount: number; orderedCount: number } | null;
  showAddCustomer: boolean;
  addCustName: string; setAddCustName: (v: string) => void;
  addCustPhone: string; setAddCustPhone: (v: string) => void;
  addCustEmail: string; setAddCustEmail: (v: string) => void;
  setCustomerFilter: (v: string) => void;
  setShowAddCustomer: (v: boolean) => void;
  loadFilteredCustomers: (filter: string) => void;
  handleAddSingleCustomer: () => void;
...
export default function CustomersTab({
  ...
  customers, customerTotal, customerFilter, customerStats,
  showAddCustomer, addCustName, setAddCustName, addCustPhone, setAddCustPhone, addCustEmail, setAddCustEmail,
  setCustomerFilter, setShowAddCustomer, loadFilteredCustomers, handleAddSingleCustomer, handleImportCSV,
  handleFullSync, handleSync,
}: CustomersTabProps) {
  return (
    <div className="space-y-5">
      {/* Sync Status Bar inline */}
      <div className="bg-slate-800 border border-white/5 rounded-2xl p-4 flex flex-wrap items-center gap-4">
        <div className="flex-1">
          <p className="text-white font-bold text-sm">Shopify Data Sync</p>
          <p className="text-slate-400 text-xs mt-0.5">{customerTotal} customers synced</p>
        </div>
        <div className="flex gap-2">
          <button onClick={handleSync} disabled={loading === "sync" || !isActive}
            ...
            {loading === "sync" ? <FaSpinner className="animate-spin" /> : "⚡"} Quick Sync
          </button>
          <button onClick={handleFullSync} disabled={loading === "fullsync" || !isActive}
            ...
            {loading === "fullsync" ? <FaSpinner className="animate-spin" /> : "🔁"} Full Sync
          </button>
        </div>
      </div>
      ...
      {customerFilter === "no_phone" && (
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 text-xs text-red-300 flex items-start gap-3">
          ...
          <div>
            ...
            <p>These customers have no phone or only an email. WhatsApp messages <strong>cannot be sent</strong> to them.</p>
          </div>
        </div>
      )}
      {customerFilter === "wa_invalid" && (
        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 text-xs text-yellow-300 flex items-start gap-3">
          ...
          <div>
            ...
            <p>Future messages to these numbers will be <strong>automatically skipped</strong> to save API quota.</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="ml-auto flex gap-2 flex-wrap">
          <a href={`${API_URL}/admin/customers/export/${merchantId}`} target="_blank" rel="noopener noreferrer"
            ...
            onClick={(e) => {
              ...
              fetch(`${API_URL}/admin/customers/export/${merchantId}`, { headers: ah() })
                .then((r) => r.blob()).then((blob) => {
                  ...
                  a.href = url; a.download = `customers-${merchantId}.csv`; a.click();
                });
            }}>⬇️ Export CSV</a>
      ...
      {showAddCustomer && (
        <div className="bg-slate-800 border border-indigo-500/20 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <input type="text" placeholder="Name" value={addCustName} onChange={(e) => setAddCustName(e.target.value)}
            ...
            <input type="tel" placeholder="Phone (e.g. 9876543210)" value={addCustPhone} onChange={(e) => setAddCustPhone(e.target.value)}
              className="p-3 bg-slate-900 border border-white/10 text-white placeholder-slate-600 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm" />
            <input type="email" placeholder="Email (optional)" value={addCustEmail} onChange={(e) => setAddCustEmail(e.target.value)}
              className="p-3 bg-slate-900 border border-white/10 text-white placeholder-slate-600 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm" />
          </div>
          <div className="flex gap-3 mt-3">
            <button onClick={handleAddSingleCustomer} disabled={loading === "addcust" || !addCustPhone}
